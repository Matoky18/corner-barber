...
const ServiceCard = ({service} : {service : Service} ) => {
      ...
      const containerStyle = `hover:shadow-lg  shadow-sm
       transition-all duration-300 ease-in-out
        p-[25px] rounded-[5px] ${flexSpacing} w-[350px] 
        h-[200px]   flex-col `

  return (
    <Link className= {` ${containerStyle}  `} href={`/services/${service.slug}`} >
   
        <h1 className= {`font-michroma text-primary-dark   border-b-3 pb-[15px] border-primary-dark `} >{service.name} : {service.duration}min</h1>
